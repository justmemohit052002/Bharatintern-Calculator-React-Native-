...
const App = () => {
  ...
  const [result,setResult] = useState('sfsjlkfjsd');
const colors = {
  dark: '#22252D',
  dark1: '#292B36',
  dark2: '#272B33',
...
<TouchableOpacity onPress={() => calculate(title)} style={{ padding:10, borderRadius:10, elevation:2,
backgroundColor: getColor(colors.light1,colors.dark2),height: 70,width: 70,margin:16}}>
...
const getColor = (light , dark) => darkTheme ? dark : light;
  return (
    <View style={{
      ...
      <Text style={{fontSize:40,color:getColor(colors.dark,colors.light), width:'100%',textAlign:'right' , padding : 20,marginTop: 20,paddingBottom:20}}>{result}</Text>
